...
function MessageBar() {
    this.show = function (className, text, interval, isHtml) {
        ...
        if ($('#' + id).length == 0)
            $('<div id="' + id + '" class="messageBar"></div>').prependTo("body").click(function () {
                messageBar.hide(className)
            });
        var $el = $('#' + id);
        if (isHtml) $el.html(text); else $el.text(text);
        $el.addClass(className).slideDown();
        if (interval) setInterval(function () {
            messageBar.hide(className);
        }, interval);
    };
...
$(function () {
    ...
    $("#AKPlan").on('mouseenter', '.fc-event', function (e) {
    ...
    if (akplan.mode != "akreadonly") {
        ...
        if (event.editable && event.avail_id) {
            menu["Löschen"] = function () {
                akplan.api.deleteRoomAvailabilityItem(event.avail_id);
            };
        }
        if (event.editable && event.termin_id) {
            menu["Aus Plan entfernen"] = function () {
                akplan.api.saveAkTerminRaw(event.termin_id, {
                    room: null,
                    start_time: null,
                    end_time: null,
                });
...
akplan.api = {
    ...
    createRoomAvailability: function (start, end, room_id) {
        ...
        } else if (akplan.mode == "availability") {
        ...
        $.ajax({
            ...
            success: function (doc) {
                ...
                for (var i in doc.events) {
                    var e = doc.events[i];
                    if (e.termin_id) {
                        e.editable = (akplan.mode == "aktermin");
                        e.color = "#000000";
                        if (akplan.mode == "akreadonly") e.url = e.view_url;
                        ...
                        switch (e.status) {
                            case STATUS.OK:
                                e.color = "#339933";
                                if (akplan.mode == "availability") {
                                    e.rendering = "";
                                }
                                if (akplan.mode == "availability" && !e.mgmt_id) {
                                    e.editable = true;
                                }
                                break;
                            case STATUS.BLOCKED:
    ...
    saveAkTerminRaw: function (terminId, postData) {
        $.ajax({
            ...
            success: function (ok) {
                console.log(ok);
                $('#calendar').fullCalendar('unselect');
                akplan.api.checkConflicts(terminId);
                $('#calendar').fullCalendar('refetchEvents');
